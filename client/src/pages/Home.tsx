...
export default function Home() {
  ...
  const markDelivered = (id: string) => {
    setOrders(data => data.map(order => order.id === id ? { ...order, status: "delivered", delivery: { completedAt: Date.now(), cost: 550000, method: "اسنپ‌باکس", note: "تحویل سالم تأیید شد." } } : order));
    setDeliveryTab("delivered"); showToast("سفارش به بخش تحویل‌داده‌شده منتقل شد.");
  };
  ...
  const DeliveryPage = <>
    ...
    <div className="delivery-list">{filteredDelivery.map(order => <article className="delivery-card" key={order.id}>
      ...
      {order.status === "pending" ? <div className="delivery-action"><div><Clock3 size={17} /><span>محصول آماده شد؟ با ثبت تحویل، سابقه و هزینه ارسال هم ثبت می‌شود.</span></div><button className="primary-button compact" onClick={() => markDelivered(order.id)}><CheckCircle2 size={17} /> تحویل داده شد</button></div> : <div className="delivery-history"><div><Truck size={17} /><span>ارسال با <strong>{order.delivery?.method}</strong> · {jalaliDate(order.delivery?.completedAt ?? order.createdAt, true)}</span></div><div><CircleDollarSign size={17} /><span>مبلغ دریافتی بابت ارسال: <strong>{money(order.delivery?.cost ?? 0)}</strong></span></div><p>{order.delivery?.note}</p></div>}
