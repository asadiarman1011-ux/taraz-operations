...
export const appRouter=router({
  ...
  users:router({list:adminProcedure.query(()=>listUsers()),update:adminProcedure.input(z.object({id:z.number().int(),data:z.object({name:z.string().min(2).max(160).optional(),jobTitle:z.string().max(120).optional(),username:z.string().min(3).max(80).optional(),password:z.string().min(6).max(200).optional(),role:z.enum(["user","admin"]).optional(),permissionsJson:z.string().nullable().optional()})})).mutation(({input})=>updateUser(input.id,input.data)),remove:adminProcedure.input(z.object({id:z.number().int()})).mutation(({input,ctx})=>deleteUser(input.id,ctx.user!.id))})
});
