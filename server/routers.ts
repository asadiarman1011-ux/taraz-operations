...
import { authenticateEmployee, createActivity, createCustomer, createEmployee, createInventoryItem, createInventoryMovement, createOrder, dismissActivity, exportDatabase, getAppSettings, getOrderDetails, listActivities, listCustomers, listInventoryItems, listInventoryMovements, listOrderHistory, listOrders, listRestoreHistory, listUsers, restoreDatabase, setAppSettings, softDeleteInventoryMovement, updateCustomer, updateInventoryItem, updateInventoryMovement, updateOrder, updateUser } from "./db";
...
export const appRouter=router({
  ...
  activities:router({list:protectedProcedure.query(({ctx})=>listActivities(ctx.user.openId)),create:protectedProcedure.input(z.object({text:z.string().min(1).max(255)})).mutation(({input})=>createActivity(input.text)),dismiss:protectedProcedure.input(z.object({id:z.number().int()})).mutation(({input,ctx})=>dismissActivity(input.id,ctx.user.openId))}),
  ...
  settings:router({get:publicProcedure.query(()=>getAppSettings()),update:editPermissionProcedure("settings.edit").input(z.record(z.string(),z.string())).mutation(({input})=>setAppSettings(input))}),
  inventory:router({
