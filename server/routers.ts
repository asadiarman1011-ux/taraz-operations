...
export const appRouter=router({
  ...
  data:router({backup:publicProcedure.query(()=>exportDatabase()),restore:publicProcedure.input(z.object({backup:z.string().min(10)})).mutation(({input})=>restoreDatabase(JSON.parse(input.backup)))}),
  inventory:router({list:publicProcedure.query(()=>listInventoryItems()),create:publicProcedure.input(z.object({kind:z.enum(["garment","material"]),dataJson:z.string()})).mutation(({input})=>createInventoryItem(input)),update:publicProcedure.input(z.object({id:z.number().int(),data:z.object({kind:z.enum(["garment","material"]).optional(),dataJson:z.string().optional()})})).mutation(({input})=>updateInventoryItem(input.id,input.data)),movements:publicProcedure.input(z.object({inventoryItemId:z.number().int()})).query(({input})=>listInventoryMovements(input.inventoryItemId)),addMovement:publicProcedure.input(z.object({inventoryItemId:z.number().int(),branchPath:z.string().max(255),direction:z.enum(["in","out","adjustment"]),quantity:z.number().int().positive(),previousQuantity:z.number().int().nullable().optional(),unit:z.string().max(40),note:z.string().nullable().optional()})).mutation(({input})=>createInventoryMovement(input)),editMovement:publicProcedure.input(z.object({id:z.number().int(),data:z.object({branchPath:z.string().max(255).optional(),direction:z.enum(["in","out","adjustment"]).optional(),quantity:z.number().int().positive().optional(),unit:z.string().max(40).optional(),note:z.string().nullable().optional(),previousQuantity:z.number().int().nullable().optional(),changeReason:z.string().min(3)} )})).mutation(({input})=>updateInventoryMovement(input.id,input.data)),deleteMovement:publicProcedure.input(z.object({id:z.number().int(),changeReason:z.string().min(3)})).mutation(({input})=>softDeleteInventoryMovement(input.id,input.changeReason))}),
