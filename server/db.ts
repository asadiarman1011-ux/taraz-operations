...
import { customers, InsertUser, orders, users } from "../drizzle/schema";
...
export async function createCustomer(input: typeof customers.$inferInsert) {
  const db = await getDb();
...
export async function updateCustomer(id: number, input: Partial<typeof customers.$inferInsert>) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
